...
export function buildUsecaseHtml(base: string): string {
  ...
  const th = (t: string) => `<th style="text-align:left;padding:8px 10px;background:#e7f3f0;border:1px solid #bfe3d7;font-size:0.9rem;white-space:nowrap">${t}</th>`;
  const td = (t: string) => `<td style="padding:8px 10px;border:1px solid #e5e7eb;font-size:0.9rem;color:#444">${t}</td>`;
  const lk = (id: string, label: string) => `<a href="${base}/${id}/" style="color:#3a8073;text-decoration:none">${label}</a>`;
